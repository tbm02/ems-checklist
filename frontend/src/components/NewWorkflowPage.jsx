...
function NewWorkflowPage() {
  ...
  const handleAddTask = () => {
    ...
    setTaskForm({ name: "", description: "", dueDate: "", assignedTo: "", sequence: tasks.length + 2 });
  };
  ...
  const handleWorkflowSubmit = (e, draft = false) => {
    ...
    setIsDraft(draft);
    // Here you would call the API to save the workflow
            ...
            <ul className="mb-2 space-y-2">
              {tasks.map((task, idx) => (
                <li key={task.id} className="flex items-center gap-2 bg-gray-50 rounded p-2">
                  ...
                  <span className="truncate max-w-xs">{task.name}, {task.description}, {task.dueDate}, Assigned to: {users.find(u => u.id == task.assignedTo)?.username || task.assignedTo}</span>
                  <button type="button" onClick={() => handleRemoveTask(task.id)} className="ml-auto text-red-500 hover:underline">Remove</button>
                </li>
              ))}
            </ul>
            <button
